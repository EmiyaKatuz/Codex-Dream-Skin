...
Linux engine for the official Codex Desktop app. It starts the official client with a loopback-only CDP port, injects the theme into `app://` renderer pages, and never modifies `app.asar` or the installed application files.
...
- The injector accepts only `app://` page targets.
