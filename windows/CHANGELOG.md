...
- 修复部分 Codex Desktop 版本在可见渲染器上调用 `Browser.getWindowForTarget` 时返回 `-32000` 或 `-32601`，导致启动验证等待约 90 秒后误判失败并回滚重启的问题；仅在严格的文档可见性、视口、结构、浏览器身份与负载校验均通过时使用兼容回退，其他异常继续安全失败。
- 在宽屏且高度充足的首页中，让备用 Command Deck 随可用高度渐进下移，避免遮挡“我们该构建什么”标题；紧凑窗口、分屏布局、背景、卡片样式与输入框位置保持不变。
## Unreleased
