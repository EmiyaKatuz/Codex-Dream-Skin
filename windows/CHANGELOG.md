...
- 将公开 v1.5.12 标签之后合入 `main` 的 Codex 26.730 兼容、运行时补丁和渲染性能修复纳入正式安装包。
...
- 修复切换会话或动态界面延迟挂载时，左侧侧栏、会话导航、工作区、环境与摘要面板及系统浮层可能短暂保持原生外观的问题；相关组件挂载后会统一在下一帧重新分类，并为会话导航提供首帧主题样式。
