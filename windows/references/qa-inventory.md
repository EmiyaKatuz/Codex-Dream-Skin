...
- Config safety: Chinese project names, LF/CRLF choice, quoted target keys, table-header comments, well-formed multiline arrays, and unrelated TOML sections survive install/selective restore; malformed arrays, ambiguous target shapes, and multiline managed settings fail unchanged, exact recovery keeps a copy of the replaced current file, and install refuses both registered and state-recorded old Codex processes.
...
- Application and tray icon: Setup, shortcuts, uninstall metadata, protocol registration, and the installed tray use the validated multi-size `assets/internet-angel-tray.ico`; its 16px through 256px frames remain recognizable and transparent.
